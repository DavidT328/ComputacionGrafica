...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, mainWindow.getposZ()));
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		glm::vec3 cochePos = glm::vec3(0.0f, 2.5f, -5.6f + mainWindow.getposZ());
		spotLights[1].SetPos(cochePos);
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Coraza.RenderModel();
		...
		glm::vec3 HPos = glm::vec3(0.0f+ mainWindow.getmuevez(), 4.722f, 6.28f);
		...
		//Faro
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(5.0f, -0.99f, -5.0f));
